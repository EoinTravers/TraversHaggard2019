...
standard.trials = get.standard.trials(erp.data)
standard.trials.modelled = filter(standard.trials, time <= -.1) # Only model up to 100 ms before action.
head(standard.trials.modelled)
```

```{r}
ggplot(erp.data, aes(time, ch1)) +
...
```{r}
quartile.rts = erp.data %>%
  group_by(speed.quantile) %>%
  summarise_at(vars(rt), .funs=list(mean=mean, sd=sd, min=min, max=max)) %>%
  mutate_if(is.numeric, round, digits=2)
quartile.rts
```

```{r fig.width=14, fig.height=3}
my.labs = as_labeller(c('1'='Slowest 25%', '2'='25% - 50%', '3'='50% - 75%', '4'='75% - 100% (Fastest 25%)'))
ggplot(erp.data) +
  facet_wrap(~speed.quantile, ncol = 4, labeller=my.labs) +
  stat_summary(mapping=aes(time, ch1), fun.y=mean, geom='path') +
  stat_summary(mapping=aes(time, ch1), fun.data=mean_se, geom='ribbon', alpha=.2) +
  geom_hline(yintercept=0, linetype='dashed') +
  geom_vline(xintercept=0, linetype='dotted') +
  geom_rect(data=quartile.rts,
            mapping=aes(xmin=-mean-sd, xmax=-mean+sd, ymin=-.2, ymax=.1),
            alpha=.3, fill='skyblue') +
  labs(x='Time to action (s)', y='Motor component (AU)') +
  # scale_y_continuous(labels=c()) +
  theme(panel.grid.major = element_line(color='black', size=.05),
        legend.key.width = unit(10, 'mm')) +
  eoinR::lgnd(0, .95)
ggsave('figures/rp_by_rt.svg', width=14, height=3)
```


# ERPs from all conditions

```{r}
subj.means = erp.data %>%
  group_by(participant, action.type, time) %>%
...
```{r}
ggplot(subj.means, aes(time, ch1, color=action.type, fill=action.type)) +
  stat_summary(fun.y=mean, geom='path') +
  stat_summary(fun.data=mean_se, geom='ribbon', alpha=.2, color=NA) +
  geom_hline(yintercept=0, linetype='dashed') +
  geom_vline(xintercept=0, linetype='dotted') +
  labs(x='Time to action (s)', y='Motor component (AU)', fill='Context', color='Context') +
  scale_y_continuous(labels=c()) +
...
```{r}
g.all = erp.data %>%
  # filter(action.type != 'Choose') %>%
  ggplot(aes(time, ch1, color=action.type, fill=action.type)) +
  stat_summary(fun.y=mean, geom='path') +
  stat_summary(fun.data=mean_se, geom='ribbon', alpha=.2, color=NA) +
  geom_hline(yintercept=0, linetype='dashed') +
  geom_vline(xintercept=0, linetype='dotted') +
...
erp.data %>%
  group_by(participant, fast, action.type, time) %>%
  summarise(ch1 = mean(ch1)) %>% 
  ggplot(aes(time, ch1, color=action.type, fill=action.type)) +
  facet_wrap(~fast) +
  stat_summary(fun.y=mean, geom='path') +
...
# Model #1: Easy vs Difficult

```{r}
.d = erp.data %>%
  filter(time < -.1) %>%
  group_by(action.type, time) %>%
  summarise(ch1=mean(ch1)) %>%
  ungroup() %>%
  summarise_at(vars(time, ch1), funs(min, max))
g.all +
  annotate('rect',
           xmin=.d$time_min,
           xmax=.d$time_max,
           ...
           ymax=.d$ch1_max,
           alpha=.2) +
  labs(title='Modelled time')
```

Median splits

```{r}
g.medians = ggplot(standard.trials, aes(time, ch1, color=iv, fill=iv, linetype=iv)) +
  stat_summary(fun.y=mean, geom='path') +
  stat_summary(fun.data=mean_se, geom='ribbon', alpha=.2, color=NA) +
  geom_hline(yintercept=0, linetype='dashed') +
  geom_vline(xintercept=0, linetype='dotted') +
  ...
  scale_y_continuous(labels=c()) +
  scale_color_manual(values=rep(c('red', 'darkgreen'), each=2)) +
  scale_fill_manual(values=rep(c('red', 'darkgreen'), each=2)) +
...
g.medians2 = ggplot(standard.trials, aes(time, ch1, color=action.type, fill=action.type)) +
  facet_wrap(~fast) +
  stat_summary(fun.y=mean, geom='path') +
  stat_summary(fun.data=mean_se, geom='ribbon', alpha=.2, color=NA) +
  geom_hline(yintercept=0, linetype='dashed') +
  ...
  eoinR::lgnd(0, 1.)
g.medians2
# ggsave('figures/rp_by_snr_x_rt.svg', width=7, height=3)
```
...
if(refit){
  ...
  spline.models = map(
    ...
    function(df) {
      ...
      m = lmer(
        ch1 ~ 1 + 
          bs(time, df=df) * z(predicted_action) * z(1/rt) + 
          z(trial_nr) +
          (1 + bs(time, 3) || participant) + 
          (1 + bs(time, 3) || unique.trial),
        data=standard.trials.modelled,
        control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
      fn = sprintf('spline.models/snr_model_df%i.rds', df)
      saveRDS(m, fn)
      cat('\n\n\n')
      return(m)
    })
...
```{r}
t = unique(standard.trials.modelled$time)
...
ivs = c('intercept', 'z(predicted_action)', 'z(1/rt)', 'z(predicted_action):z(1/rt)')
ivs = c('intercept', 'z(predicted_action)', 'z(1/rt)', 'z(predicted_action):z(1/rt)')
nice.ivs = c('Intercept', 'P(Act)', 'Response speed', 'P(Act) x Speed')
names(nice.ivs) = ivs
names(ivs) = nice.ivs
...
fixef.effect.estimates = map_df(ivs, function(iv){
  print(iv)
  res = get.marginal.fx(standard.trials.modelled, sims, var.name=iv, spline.df=5)
  res$variable = iv
  return(res)
})
...
ggplot(fixef.effect.estimates, aes(t, val)) +
  ...
  facet_wrap(~Term) +
  stat_summary(fun.data=mean_sdl, geom='ribbon', alpha=.9, fill='skyblue') +
  stat_summary(fun.y=mean, geom='path') +
  geom_hline(yintercept=0, linetype='dashed') +
  theme(legend.position='none') +
  labs(x='Time to action (s)', y='Regression weight')
```
...
marginal.splines = map_df(ivs, function(iv){
  get.marginal.splines(standard.trials.modelled, sims, iv) %>%
    mutate(variable = iv)
})
...
ggplot(marginal.splines,  aes(t, val, color=basis, fill=basis, group=basis)) +
  ...
  facet_wrap(~Term) +
  theme(legend.position='none') +
  labs(x='Time to action (s)', y='Spline regression weight', color='Basis', fill='Basis')
```

### Model predictions

```{r fig.width=7, fig.height=3}
pred.df = get.predictions(standard.trials.modelled, sims, m.spline)
